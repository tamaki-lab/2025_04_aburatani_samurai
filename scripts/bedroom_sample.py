...
def visualize_and_save(video_folder, prompts, model_cfg, checkpoint, vis_folder, save_to_video=True):
    ...
    with torch.inference_mode(), torch.autocast("cuda", dtype=torch.float16):
        ...
        # 最初の2つのプロンプト (バウンディングボックス) を使って追跡開始
        for fid, (bbox, track_label, start_frame) in enumerate(prompts):
            frame_idx, object_ids, masks = predictor.add_new_points_or_box(state, box=bbox, frame_idx=start_frame, obj_id=fid)
...
prompts = [
    ((300, 0, 500, 400), 0, 0),  # 1番目のオブジェクト
    ((200, 100, 500, 450), 1, 100),  # 2番目のオブジェクト
]
